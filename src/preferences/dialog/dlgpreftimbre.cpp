...
DlgPrefTimbre::DlgPrefTimbre(QWidget *parent, UserSettingsPointer pConfig)
...
void DlgPrefTimbre::setDefaults() {
    ...
    m_bReanalyzeEnabled = false;
    m_selectedAnalyser = VAMP_ANALYZER_TIMBRE_DEFAULT_PLUGIN_ID;
        ...
        if (plugin) {
            ...
            for (unsigned int iOutput=0; iOutput < outputs.size(); iOutput++) {
                ...
                bool isTimbreDetector = displayName.contains(
                    VAMP_ANALYZER_TIMBRE_DEFAULT_PLUGIN_ID);
                if (isTimbreDetector) {
...
void DlgPrefTimbre::loadSettings() {
    ...
    if(m_pConfig->getValueString(
        ConfigKey(VAMP_CONFIG_KEY, VAMP_ANALYZER_TIMBRE_PLUGIN_ID))==QString("")) {
        setDefaults();
