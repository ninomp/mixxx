...
#include "analyzertimbre.h"
#include "track/timbre_preferences.h"
#include "track/timbrefactory.h"


AnalyzerTimbre::AnalyzerTimbre(UserSettingsPointer pConfig)
    : m_pConfig(pConfig),
      m_pVamp(NULL),
      m_iSampleRate(0),
      m_iTotalSamples(0),
...
AnalyzerTimbre::~AnalyzerTimbre() {
}

bool AnalyzerTimbre::initialize(TrackPointer tio, int sampleRate, int totalSamples) {
    m_bShouldAnalyze = false;
    if (totalSamples == 0) {
        return false;
    }
    ...
    QString library = m_pConfig->getValueString(
        ConfigKey(VAMP_CONFIG_KEY, VAMP_ANALYZER_TIMBRE_LIBRARY));
    QString pluginID = m_pConfig->getValueString(
        ConfigKey(VAMP_CONFIG_KEY, VAMP_ANALYZER_TIMBRE_PLUGIN_ID));
...
void AnalyzerTimbre::process(const CSAMPLE *pIn, const int iLen) {
...
bool AnalyzerTimbre::isDisabledOrLoadStoredSuccess(TrackPointer tio) const {
    return checkStoredTimbre(tio);
}

void AnalyzerTimbre::cleanup(TrackPointer tio) {
    Q_UNUSED(tio);
    delete m_pVamp;
    m_pVamp = NULL;
}

void AnalyzerTimbre::finalize(TrackPointer tio) {
    if (!m_bShouldAnalyze || m_pVamp == NULL) {
        return;
    }
    ...
    QStringList sl;
    foreach (double value, timbreVector) {
        sl << QString::number(value);
    }
    qDebug() << "Timbre vector:" << sl.join(" ");
    ...
    qDebug() << "Timbre version" << pTimbre->getVersion()
             << "serialized size" << pTimbre->toByteArray().length();
    tio->setTimbre(pTimbre);
}

QHash<QString, QString> AnalyzerTimbre::getExtraVersionInfo(
...
bool AnalyzerTimbre::checkStoredTimbre(TrackPointer tio) const {
    bool isStored = false;
