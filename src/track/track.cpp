...
const CuePosition Track::kDefaultLoadCuePosition = CuePosition(0.0, Cue::AUTOMATIC);
...
void Track::initId(TrackId id) {
    ...
    // The id is also stored in each cue point!
    DEBUG_ASSERT(m_cuePoints.empty());
...
// Ensures that each track always has exactly one load cue!
CuePointer Track::refreshLoadCue(CuePosition defaultPos) {
    ...
    pLoadCue->setCuePosition(defaultPos);
    connect(pLoadCue.get(), &Cue::updated, this, &Track::slotCueUpdated);
    m_cuePoints.push_back(pLoadCue);
    return pLoadCue;
}

CuePointer Track::getOrAddLoadCue(CuePosition defaultPos) {
    QMutexLocker lock(&m_qMutex);
    return refreshLoadCue(defaultPos);
}

void Track::setCuePoint(CuePosition pos) {
    QMutexLocker lock(&m_qMutex);
    CuePointer pLoadCue = refreshLoadCue();
    lock.unlock();
    ...
    pLoadCue->setCuePosition(pos);
}

CuePosition Track::getCuePoint() const {
    CuePointer pLoadCue = findCueByType(Cue::LOAD);
    VERIFY_OR_DEBUG_ASSERT(pLoadCue) {
        return kDefaultLoadCuePosition;
    }
...
CuePointer Track::findCueByType(Cue::CueType type) const {
    // This method cannot be used for hotcues or loops because there can be
    // multiple hotcues/loops and this function returns only a single CuePointer.
    DEBUG_ASSERT(type != Cue::CUE);
    DEBUG_ASSERT(type != Cue::LOOP);
    QMutexLocker lock(&m_qMutex);
...
QList<CuePointer> Track::getCuePoints() const {
    // Copying an implicitly shared object is thread-safe
    return m_cuePoints;
}

void Track::setCuePoints(QList<CuePointer> cuePoints) {
    ...
    m_cuePoints = std::move(cuePoints);
