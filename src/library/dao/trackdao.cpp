...
namespace {
    ...
    void bindTrackLibraryValues(QSqlQuery* pTrackLibraryQuery, const Track& track) {
        ...
        QByteArray timbreBlob;
        QString timbreVersion;
        QString timbreSubVersion;
        const TimbrePointer pTimbre(track.getTimbre());
        if (!pTimbre.isNull()) {
            qDebug() << "Track has valid timbre.";
            timbreBlob = pTimbre->toByteArray();
            timbreVersion = pTimbre->getVersion();
            timbreSubVersion = pTimbre->getSubVersion();
        }
        pTrackLibraryQuery->bindValue(":timbre", timbreBlob);
        pTrackLibraryQuery->bindValue(":timbre_version", timbreVersion);
        pTrackLibraryQuery->bindValue(":timbre_sub_version", timbreSubVersion);
    }
...
bool setTrackTimbre(const QSqlRecord& record, const int column,
                    TrackPointer pTrack) {
    QByteArray timbreBlob = record.value(column).toByteArray();
    QString timbreVersion = record.value(column + 1).toString();
    QString timbreSubVersion = record.value(column + 2).toString();
...
TrackPointer TrackDAO::getTrackFromDB(TrackId trackId) const {
    ...
    ColumnPopulator columns[] = {
        ...
        // Timbre columns are handled by setTrackTimbre. Do not change the
        // ordering of these columns or put other columns in between them!
        { "timbre", setTrackTimbre },
        { "timbre_version", nullptr },
        { "timbre_sub_version", nullptr },
...
bool TrackDAO::updateTrack(Track* pTrack) {
    ...
    query.prepare("UPDATE library SET "
            ...
            "keys=:keys,"
            "timbre=:timbre,"
            "timbre_version=:timbre_version,"
            "timbre_sub_version=:timbre_sub_version,"
